...
export default {
  name: 'List',
  components: {
    standardColorEdit: resolve => {
      require(['./AddOrEdit'], resolve)
    }
  },
  created: function () {
    bus.$on(productStandardColor.search, (searchData) => { // 监听外部查询数据事件
      this.search(searchData)
    })
    bus.$on(productStandardColor.edit, () => { // 监听数据更改后的列表刷新（刷新当前页）
      this.getStandardColor()
    })
    bus.$on(productStandardColor.add, () => { // 监听数据添加后的列表刷新
      this.resetPageInfo()
      this.getStandardColor()
    })
  },
  data: function () {
    let pageSize = localStorage.getItem(localStorageKeys.productColorSize)
    return {
      editCompName: '',
      showEdit: false,
      editRowId: '',
  ...
  methods: {
    ...
    getStandardColor: function () {
      this.$http.post('/standardColor/getStandardColorsByPage', Object.assign({}, this.searchData, this.pageInfo)).then((response) => {
    ...
    handleSizeChange: function (val) { // 每页条数改变时重新加载记录
      ...
      localStorage.setItem(localStorageKeys.productColorSize, val)
      this.getStandardColor()
    },
    add: function () { // 添加记录
      this.editCompName = 'standardColorEdit'
      this.editRowId = null
      this.showEdit = true
    },
    edit: function (item) {
    ...
    _setStatus: function (status, selectIds) {
      this.$http.post('/standardColor/delColors', {ids: selectIds, status: status}).then((response) => {
        this.getStandardColor()
